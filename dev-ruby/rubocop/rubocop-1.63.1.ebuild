...
all_ruby_prepare() {
	...
	# Avoid mandatory usage of bundler. This got introduced in
	# https://github.com/rubocop/rubocop/pull/12826/commits/5d696422df1ee3b1dc90928470402a30a2f8f3b3
	# but this looks wrong: if a project does not use bundler it should
	# not be forced to use it in this way and lead to failure due to
	# missing Gemfiles.
	sed -e '/require.*bundler/ s:^:#:' \
		-i lib/rubocop/lockfile.rb || die
}
