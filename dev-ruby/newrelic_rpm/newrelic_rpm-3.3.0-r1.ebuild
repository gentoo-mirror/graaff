...
all_ruby_prepare() {
	epatch "${FILESDIR}/${PN}-3.3.0-no-webrick.patch"

	# Remove useless log message that puts itself on stdout for every
	# rake task in production.
	sed -i -e '/New Relic Agent not running/ s:^:#:' lib/new_relic/control/frameworks/rails.rb || die
}
